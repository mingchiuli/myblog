...
public class ScheduledTask {
    ...
    public ScheduledTask(@Qualifier("scheduledThreadPoolExecutor") ThreadPoolExecutor executor, BlogService blogService, RedisTemplate<String, Object> redisTemplate, RedisLock redisLock) {
    ...
    public void configureTask() {
        //分布式锁
        boolean tryLock = redisLock.tryLock(scheduling, scheduling, 100, 10000);
        ...
        try {
            if (Boolean.FALSE.equals(redisTemplate.hasKey(scheduled))) {
                ...
                redisTemplate.opsForValue().set(scheduled, scheduled, 10, TimeUnit.MINUTES);

                log.info("定时任务执行完毕");
            }
        } finally {
            redisLock.unLock(scheduling, scheduling);
        }
    }
}
